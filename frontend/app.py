...
def api_batch_analyze(url: str) -> dict | None:
    try:
        resp = requests.post(
            f"{BACKEND_URL}/api/batch-analyze",
            json={"url": url},
            timeout=600,   # no cap – may take a while for large searches
        )
        if resp.status_code == 200:
            return resp.json()
        st.error(f"Chyba API ({resp.status_code}): {resp.json().get('detail', resp.text)}")
...
if page == "Analyzovat nemovitost":
    ...
    st.markdown("Vložte URL nemovitosti ze Sreality nebo Bezrealitky, nebo vyplňte parametry ručně.")

    tab_url, tab_manual, tab_batch = st.tabs(["📎 Vložit URL", "✏️ Ruční zadání", "🔎 Hromadná analýza"])
    ...
    with tab_url:
        ...
        st.markdown("---")
        if st.button("🔍 Analyzovat", type="primary", use_container_width=True, key="analyze_url"):
            if not payload:
                st.warning("Zadejte URL nemovitosti.")
            else:
                with st.spinner("Probíhá analýza…"):
                    _res = api_analyze(payload)
                if _res:
    ...
    with tab_manual:
        ...
        with c1:
            m_price = st.number_input("Cena (Kč) *", min_value=0, step=50000, value=0, key="m_price")
            m_size = st.number_input("Plocha (m²) *", min_value=0.0, step=1.0, value=0.0, key="m_size")
        with c2:
            m_city = st.text_input("Město *", key="m_city")
            m_district = st.text_input("Okres", key="m_district")
        with c3:
            m_disp = st.selectbox(
                "Dispozice",
            ...
            m_address = st.text_input("Adresa", key="m_address")

        st.markdown("**Kvalita nemovitosti**")
        c4, c5, c6 = st.columns(3)
        with c4:
            m_construction = st.selectbox(
                "Konstrukce",
                ["", "cihla", "panel", "smiseny", "nizkoenergeticky", "drevostavba", "montovany"],
                key="m_construction",
            )
        with c5:
            m_energy = st.selectbox(
                "Energetická třída (PENB)",
                ["", "A", "B", "C", "D", "E", "F", "G"],
                key="m_energy",
            )
        with c6:
            m_year = st.number_input("Rok výstavby", min_value=1850, max_value=2025, value=2000, key="m_year")
            m_floor = st.number_input("Podlaží", min_value=0, max_value=50, value=0, key="m_floor")

        st.markdown("**Lokalita a vlastnictví** *(klíčové pro zástavní hodnotu)*")
        c7, c8, c9 = st.columns(3)
        with c7:
            m_ownership = st.selectbox(
                "Vlastnictví",
                ["", "OV", "DV", "DV_no_transfer"],
                format_func=lambda x: {
                    "": "— (neznámé)",
                    "OV": "OV — osobní vlastnictví",
                    "DV": "DV — družstevní",
                    "DV_no_transfer": "DV — bez možnosti převodu",
                }[x],
                key="m_ownership",
            )
            m_locality_tier = st.selectbox(
                "Lokalita tier",
                ["", 1, 2, 3],
                format_func=lambda x: {
                    "": "— (neznámý)",
                    1: "T1 — dobrá zástavní hodnota",
                    2: "T2 — průměrná lokalita",
                    3: "T3 — problematická lokalita",
                }[x],
                key="m_locality_tier",
            )
        with c8:
            m_svl_risk = st.selectbox(
                "SVL riziko",
                ["", "none", "proximity", "direct"],
                format_func=lambda x: {
                    "": "— (neznámé)",
                    "none": "Bez rizika",
                    "proximity": "Blízkost SVL",
                    "direct": "Přímá SVL",
                }[x],
                key="m_svl_risk",
            )
            m_service_charge = st.number_input(
                "Fond oprav (Kč/měsíc)", min_value=0, step=100, value=0, key="m_service_charge",
            )
        with c9:
            m_building_revitalized = st.selectbox(
                "Revitalizace domu",
                ["", True, False],
                format_func=lambda x: {
                    "": "— (neznámé)",
                    True: "Ano — revitalizovaný",
                    False: "Ne — nerevitalizovaný",
                }[x],
                key="m_building_revitalized",
            )

        # Build manual payload
        if not payload.get("url"):
            if m_price > 0:
                payload["price"] = float(m_price)
            if m_size > 0:
                payload["size_m2"] = m_size
            if m_city:
                payload["city"] = m_city
            if m_district:
                payload["district"] = m_district
            if m_address:
                payload["address"] = m_address
            if m_disp:
                payload["disposition"] = m_disp
            if m_construction:
                payload["construction_type"] = m_construction
            if m_energy:
                payload["energy_class"] = m_energy
            if m_year and m_year != 2000:
                payload["year_built"] = m_year
            if m_floor > 0:
                payload["floor"] = m_floor
            # Nová pole
            if m_ownership:
                payload["ownership"] = m_ownership
            if m_locality_tier != "":
                payload["locality_tier"] = int(m_locality_tier)
            if m_svl_risk:
                payload["svl_risk"] = m_svl_risk
            if m_service_charge > 0:
                payload["service_charge"] = float(m_service_charge)
            if m_building_revitalized != "":
                payload["building_revitalized"] = bool(m_building_revitalized)

        st.markdown("---")
        if st.button("🔍 Analyzovat", type="primary", use_container_width=True, key="analyze_manual"):
            if not payload:
                st.warning("Zadejte parametry nemovitosti.")
            else:
                with st.spinner("Probíhá analýza…"):
                    _res = api_analyze(payload)
                if _res:
    ...
    with tab_batch:
        st.markdown(
            "Vložte URL výsledků vyhledávání ze Sreality. Aplikace projde všechny stránky, "
            "stáhne detaily každého inzerátu a ohodnotí je skóre."
        )
        batch_url = st.text_input(
            "URL vyhledávání (Sreality)",
            placeholder="https://www.sreality.cz/hledani/prodej/byty/teplice?velikost=2%2B1...",
            key="batch_url_input",
        )
        if batch_url and "lat-max" not in batch_url:
            st.info(
                "**Tip:** URL neobsahuje souřadnice mapy. Aplikace filtruje na úrovni kraje, "
                "nikoli konkrétního města — výsledků může být více než na webu. "
                "Pro přesné výsledky použijte URL z **mapového zobrazení** Sreality "
                "(přepněte na mapu, nastavte oblast, zkopírujte URL)."
            )
        st.caption("Aplikace projde všechny stránky výsledků a stáhne každý inzerát. U většího vyhledávání to může trvat déle (~0.35 s/nemovitost).")
        st.markdown("---")
        batch_btn = st.button("🚀 Spustit hromadnou analýzu", type="primary", use_container_width=True)

        if batch_btn:
            if not batch_url:
                st.warning("Zadejte URL vyhledávání.")
            elif "sreality" not in batch_url.lower():
                st.error("Hromadná analýza momentálně podporuje pouze Sreality.")
            else:
                import threading, time as _time

                _result_box: dict = {"result": None, "done": False}

                def _run():
                    _result_box["result"] = api_batch_analyze(batch_url)
                ...
                while not _result_box["done"]:
                    ...
                    import math
                    status_ph.caption(f"⏳ Scrapuji a hodnotím inzeráty…  uplynulo {elapsed:.0f} s")
                    _tick += 1
                    _time.sleep(0.5)

                _thread.join()
                status_ph.caption("✅ Hotovo!")
                result = _result_box["result"]

                if result:
                    skipped = result.get("total_skipped", 0)
                    skip_note = f" · Přeskočeno (již v DB): **{skipped}**" if skipped else ""
                    total_matching = result.get("total_matching") or result["total_found"]
                    st.success(
                        f"Hotovo! Nalezeno: **{total_matching}** · "
                        f"Staženo: **{result['total_scraped']}** · "
                        f"Uloženo: **{result['total_saved']}**{skip_note}"
                    )
                    if result["total_saved"] == 0 and skipped > 0:
                        st.info("Všechny nalezené inzeráty jsou již v historii analýz. Zkuste jiné vyhledávání nebo smažte historii.")

                    if result.get("errors"):
                        with st.expander(f"Chyby ({len(result['errors'])})"):
                            for err in result["errors"]:
                                st.caption(err)

                    props = result.get("properties", [])
                    if props:
                        st.subheader("Výsledky seřazené podle skóre")
                        rows = []
                        for p in props:
                            score = p.get("score_total") or 0
                            rows.append({
                                "Skóre": score,
                                "Adresa": p.get("address") or p.get("city") or "—",
                                "Dispozice": p.get("disposition") or "—",
                                "Cena": _fmt_czk(p.get("price")),
                                "Plocha": f"{p['size_m2']:.0f} m²" if p.get("size_m2") else "—",
                                "Výnos": _fmt_pct(p.get("gross_yield_pct")),
                                "URL": p.get("url") or "",
                            })
                        import pandas as pd
                        ...
                        st.dataframe(
                            df.style.map(color_score, subset=["Skóre"]),
                            use_container_width=True,
                            height=min(50 + len(rows) * 35, 600),
                        )

                        st.subheader("Top 3 nemovitosti")
                        for p in props[:3]:
                            score = p.get("score_total") or 0
                            color = _color_for_score(score)
                            addr = p.get("address") or p.get("city") or "—"
                            url_link = f"<a href='{p['url']}' target='_blank'>Otevřít ↗</a>" if p.get("url") else ""
                            st.markdown(
                                f"""<div style="border-left:5px solid {color}; padding:10px 16px;
                                        background:{color}11; border-radius:4px; margin-bottom:8px;">
                                    <b style="color:{color}; font-size:1.1rem">{score:.0f}/100</b>
                                    &nbsp; {addr}
                                    &nbsp; {p.get('disposition','')}&nbsp;
                                    &nbsp; {_fmt_czk(p.get('price'))}
                                    &nbsp; výnos {_fmt_pct(p.get('gross_yield_pct'))}
                                    &nbsp; {url_link}
                                </div>""",
                                unsafe_allow_html=True,
